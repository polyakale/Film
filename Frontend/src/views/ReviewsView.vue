      ...
      <div class="data-container">
        <div v-show="isLoggedIn && activeTab === 'my'" class="table-section">
          <div class="table-wrapper" ref="tableWrapperRef">
             <div v-if="!paginatedFavourites.length && filteredFavourites.length === 0 && !searchQuery" class="no-data-message">
        ...
        <div v-show="activeTab === 'all' || !isLoggedIn" class="public-reviews-section">
          <div class="reviews-wrapper" ref="reviewsWrapperRef">
            <div v-if="!paginatedPublicReviews.length && filteredPublicReviews.length === 0 && !searchQuery" class="no-data-message">
            ...
            <div v-else class="reviews-grid">
              ...
              >
                ...
                <div class="review-film-title">{{ review.filmTitle || "Unknown Film" }}</div>
                <div class="review-content">{{ review.content /* Use pre-processed content */ }}</div>
                <div class="star-rating d-inline-flex align-items-center">
                  <i
                    v-for="starIndex in 5"
...
import { BASE_URL } from "../helpers/baseUrls"; // Adjust path as needed

// Import Child Components
import Modal from "@/components/Modal.vue"; // Adjust path as needed
import Paginator from "@/components/Paginator.vue"; // Adjust path as needed
import OperationsCrud from "@/components/OperationsCrud.vue"; // Adjust path as needed
import ReviewForm from "@/components/forms/ReviewForm.vue"; // Adjust path as needed
...
export default {
  ...
  data() {
    return {
      ...
      showTableFilter: false, // Controls visibility of 'my' reviews filter
      showPublicFilter: false, // Controls visibility of 'all' reviews filter
  ...
  computed: {
     ...
     */
    filteredFavourites() {
        return this.filterAndSortReviews(this.favourites, this.searchQuery, this.tableReviewFilter, false);
    },
    ...
    filteredPublicReviews() {
        return this.filterAndSortReviews(this.publicReviews, this.searchQuery, this.publicReviewFilter, true);
    },
    ...
    totalFavPages() {
        ...
        return Math.ceil(this.filteredFavourites.length / this.dynamicItemsPerPage);
    },
    ...
    totalPublicPages() {
         if (!this.dynamicItemsPerPage) return 0;
         return Math.ceil(this.filteredPublicReviews.length / this.dynamicItemsPerPage);
    },
     ...
     */
    publicPagesArray() {
        return this.generatePagesArray(this.totalPublicPages, this.currentPublicPage);
    },
  },
  methods: {
    /**
    ...
    async _fetchApi(url, config = {}, method = 'get', data = null) {
        ...
        try {
            ...
            this.errorMessages = `Operation failed: ${error.response?.data?.message || error.message || 'An unknown error occurred.'}`;
            return null;
        } finally {
            this.loading = false;
        }
     ...
     */
    toggleControl(type) {
        const isOpening = (type === 'search' && !this.showSearch) ||
                          (type === 'tableFilter' && !this.showTableFilter) ||
                          (type === 'publicFilter' && !this.showPublicFilter);

        this.showSearch = (type === 'search') ? !this.showSearch : (isOpening ? false : this.showSearch);
        this.showTableFilter = (type === 'tableFilter') ? !this.showTableFilter : (isOpening ? false : this.showTableFilter);
        this.showPublicFilter = (type === 'publicFilter') ? !this.showPublicFilter : (isOpening ? false : this.showPublicFilter);
    },
    ...
    onClickDelete(item) {
        ...
        this.modalMessageYesNo = `Are you sure you want to delete your review for "${item.filmTitle || 'this film'}"? This action cannot be undone.`;
    ...
    async saveItemHandler(formData) {
        ...
        // On error, modal remains open for user to cancel or retry
    },
    ...
    closeModal() {
        ...
        // Reset modal state *after* it's confirmed closed/hidden
        ...
        this.modalNo = null;
        this.modalSize = null;
        // Decide if errors should be cleared on manual close
        // this.errorMessages = null;
    },
    ...
    if (query) {
        ...
        switch (filterType) {
            ...
            case "lowToHigh":
                ...
                break;
            case "newest":
                if (!isPublic) filtered.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
                break;
            case "oldest":
                 if (!isPublic) filtered.sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
                break;
        }
        return filtered;
    },
    ...
    calculateDimensions() {
        ...
        if (containerHeight > 0 && itemHeight > 0) {
            this.dynamicItemsPerPage = Math.max(1, Math.floor(containerHeight / itemHeight));
        } else {
...
.filter-wrapper { min-width: 200px; }
...
.table-section, .public-reviews-section {
...
.custom-table .film { font-weight: 500; }
.custom-table .date { font-size: 0.9em; color: #aaa; }

/* Star Rating Styles */
.star-rating { font-size: 1.1rem; } /* Adjust size */
...
.review-author { font-weight: 600; color: #ccc; }
.review-film-title {
  font-size: 1.1rem;
  font-weight: 600;
...
@media (max-width: 768px) {
  ...
  .custom-table thead { display: none; /* Hide table header on small screens */ }
  .custom-table tbody, .custom-table tr, .custom-table td { display: block; width: 100%; }
  .custom-table tr {
  ...
  .custom-table td::before {
   ...
   .custom-table td.text-center { text-align: right; } /* Override center alignment */
   .custom-table td.text-center::before { text-align: left; } /* Keep label left */
